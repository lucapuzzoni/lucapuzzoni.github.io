---
layout: default
---

<h1>News</h1>

<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - Runner - Writeup</h2>
        <p>Season 5 is started on HackTheBox. A new writeup for HackTheBox - Runner is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/machiens/runner/runner'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>25 Apr 2024</small>
      </div>
  </div>
</article>


<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - Insomnia - Writeup</h2>
        <p>A new writeup for HackTheBox - Insomnia is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/challenges/insomnia/insomnia'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>25 Apr 2024</small>
      </div>
  </div>
</article>


<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - JSCalc - Writeup</h2>
        <p>A new writeup for HackTheBox - JSCalc is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/challenges/jscalc/jscalc'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>25 Apr 2024</small>
      </div>
  </div>
</article>


<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - PDFy - Writeup</h2>
        <p>A new writeup for HackTheBox - PDFy is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/challenges/PDFy/pdfy'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>25 Apr 2024</small>
      </div>
  </div>
</article>

<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - CozyHosting - Writeup</h2>
        <p>A new writeup for HackTheBox - CozyHosting is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/machines/cozyhosting'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>31 Dec 2023</small>
      </div>
  </div>
</article>


<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - Devvortex - Writeup</h2>
        <p>A new writeup for HackTheBox - Devvortex is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/machines/devvortex'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>29 Dec 2023</small>
      </div>
  </div>
</article>

<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - Sau - Writeup</h2>
        <p>A new writeup for HackTheBox - Sau is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/machines/sau'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>10 Dec 2023</small>
      </div>
  </div>
</article>

<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - Keeper - Writeup</h2>
        <p>A new writeup for HackTheBox - Keeper is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/machines/keeper'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>07 Dec 2023</small>
      </div>
  </div>
</article>

<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - Neonify - Writeup</h2>
        <p>A new writeup for HackTheBox - Neonify is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/challenges/neonify'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>04 Dec 2023</small>
      </div>
  </div>
</article>

<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - Toxic - Writeup</h2>
        <p>A new writeup for HackTheBox - Toxic is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/challenges/toxic'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>04 Dec 2023</small>
      </div>
  </div>
</article>

<hr>
<article>
  <div style="display: flex;">
      <div style="flex: 1">
        <center>
          <img width="400em" src="data:image/png;base64," alt="">
        </center>
      </div>
      <div style="flex: 1">
        <h2>HackTheBox - LoveTok - Writeup</h2>
        <p>A new writeup for HackTheBox - LoveTok is now Available.</p>
        
        <center>
          <a href="{{'/pages/writeups/hackthebox/challenges/lovetok'}}" class="btn">Check it Out</a>
        </center>

        <br>
        <small>04 Dec 2023</small>
      </div>
  </div>
</article>